...
def admin_menu():
    ...
    btn = function.place_button(panel_option_admin, "Add", "Blue", lambda: add_movie(
        window_admin, movie_name, movie_genre, movie_director, movie_rating, movie_synopsis ), 20, 100)
    btn = function.place_button(panel_option_admin, "Reset", "Red", lambda: reset_movie(
        movie_name, movie_genre, movie_director, movie_rating, movie_synopsis), 60, 100)

    btn = function.place_button(panel_option_admin, "Remove Selected", "Red", lambda: del_movie(
        catalog_movie_admin, window_admin), 100, 100)
    window_admin.mainloop()


def add_movie(window_admin: Misc, movie: Entry, genre: Entry, director: Entry, rating: Entry,synopsis: Entry):
    ...
    with open("database/movies.csv", "a", encoding="UTF-8") as f:  # append the new data
        f.write(save)
    messagebox.showinfo(title="Sucess", message="Movie successfully added", parent=window_admin)
    window_admin.destroy()
    admin_menu()


def del_movie(catalog_movie_admin: ttk.Treeview, window_admin: Misc):
    ...
    with open("database/movies.csv", "w", encoding="UTF-8") as f:
        f.write(new_text)
    messagebox.showinfo(title="Sucess", message="Movie successfully deleted", parent=window_admin)
    window_admin.destroy()
    admin_menu()


def reset_movie(movie: Entry, genre: Entry, director: Entry, rating: Entry, synopsis: Entry):
    movie.delete(0, END)
    genre.delete(0, END)
