    ...
    with open("users.csv", "r", encoding="UTF-8") as f:             
        for line in f:
            ...
            if user_id == user_line[0]:
                first_name = user_line[1]           #writes a label on the perfil saying the users first name
                last_name = user_line[2]           #writes a label on the perfil saying the users last name
                email = user_line[3]            #writes a label on the perfil saying the users email
    
    lbl_fname = function.place_label(perfil_window, "First Name: ", 10, 10, "grey")
    lbl_lname = function.place_label(perfil_window, "Last Name: ", 20, 20, "grey")
...
def last_session(user_id):         #writes on the "users.csv" the last time the user was online (Time/Date)LL
    ...
    with open("database/users.csv", "r", encoding="UTF-8") as f:
        for line in f:
            ...
            if words[5] == "admin" and user_id == words[0]:
                Menu_bar.add_command(label="Admin Configs", command=admin.admin_menu)
